...
from mixins import DistributedDataloader, LoraMixin, SubsampleDataset
from utils import clamp_preserve_grad, concat, gather, iter_dict, ungather
...
class Trainer(BaseTrainer, LoraMixin):
    ...
    def __init__(self, config):
        ...
        # all_data is the task's own indices, so what comes off the dataloader is data_ids
        self.train_dataset = SubsampleDataset(all_data=range(len(self.sample_task.data)),N=config.sample.total_samples,m=config.sample.m,base_seed=config.seed,)
        self.training_dataloader = DistributedDataloader(self.train_dataset, num_processes=self.accelerator.num_processes, process_index=self.accelerator.process_index, batch_size=config.model.max_batch_size_per_device)

        self.val_dataset = SubsampleDataset(all_data=range(len(self.val_task.data)),N=-1,k=config.val.k,base_seed=config.seed,)
        self.val_dataloader = DistributedDataloader(self.val_dataset, num_processes=self.accelerator.num_processes, process_index=self.accelerator.process_index, batch_size=config.model.max_batch_size_per_device)
    ...
    def sampling_step(self, epoch):
        ...
        gathered_data_ids = gather(training_data["data_ids"]).tolist()
        gathered_rewards = gather(training_data["rewards"])
        gathered_entropy = gather(training_data["entropies"])
        gathered_length = gather(training_data["generated_lengths"])
        gathered_texts = gather(training_data["generated_texts"])
        gathered_means, gathered_stds = self.compute_reward_statistics(gathered_data_ids, gathered_rewards)
        training_data["reward_means"] = ungather(gathered_means, len(training_data["rewards"]), self.accelerator.process_index)   # (N_local,)
        training_data["reward_stds"] = ungather(gathered_stds, len(training_data["rewards"]), self.accelerator.process_index)     # (N_local,)
    ...
    def validation_step(self, epoch):
        ...
        gathered_data_ids = gather(val_data["data_ids"]).tolist()
        gathered_rewards = gather(val_data["rewards"])
        gathered_entropy = gather(val_data["entropies"])
        gathered_texts = gather(val_data["generated_texts"])
