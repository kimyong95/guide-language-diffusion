...
class Trainer(BaseTrainer):
    """Hidden-state policy optimization: Lx hidden-state vectors injected into every transformer layer's KV
    ...
    def __init__(self, config):
        ...
        # Per-layer KV-noise hidden states x: (H, Lx, D). Seeded off a CPU generator so every rank
        ...
        x = project_to_sphere(torch.randn(H, Lx, D, generator=gen))
        ...
        self.train_dataset = DistributedSubsampleDataset(
            ...
            G=G,
            m=config.sample.m,
            N_batch_max=config.sample.total_samples,
    ...
    def build_noisy_cache(self, prompt_tokens):
        ...
        # produced by pushing this parameter's hidden state x[h] through that layer's real K/V pipeline
        # (input_layernorm -> k/v_proj -> k_norm -> RoPE), landing the keys/values on the model's true
        # K/V manifold (cf. test-noise.py Stage 2). The same noise x is broadcast over the (N,) batch.
        # Differentiable in x; wrap the call in torch.no_grad when the graph is not wanted. Returns
        # (cache, prompt_logits, Lp).
        Lx = self.config.sample.noise_length
        ...
        for h, layer in enumerate(self.model.model.layers):
            ...
            Dh = attn.head_dim
            x = self.x[h][None].to(dev, attn.k_proj.weight.dtype).expand(N, -1, -1)  # (N, Lx, D)
            ...
            k = einops.rearrange(k, "b l h d -> b h l d")               # (N, heads_kv, Lx, Dh)
            v = einops.rearrange(attn.v_proj(xln), "b l (h d) -> b h l d", d=Dh)  # (N, heads_kv, Lx, Dh)
    ...
    def generate(self, prompt_tokens):
        ...
        # forbids position Lp-1 from seeing rows at >=Lp); every later token is sampled at position >= Lp+Lx
        # and attends over the noise KV. Padding the input to Lp+Lx+1 makes `generate` feed only first_token
        # (at position Lp+Lx) and take over -- the Lx placeholders stand in for the injected KV rows and are
        # never embedded (cf. test-noise.py).
        cfg = self.config.sample
        Lx = cfg.noise_length
        k = prompt_tokens.shape[0]
        cache, prompt_logits, Lp = self.build_noisy_cache(prompt_tokens)
        first_token = prompt_logits[:, -1:, :].argmax(-1)  # (k, 1), identical within the group
        placeholders = prompt_tokens.new_full((k, Lx), self.tokenizer.pad_token_id)
        input_ids = torch.cat([prompt_tokens, placeholders, first_token], dim=1)  # (k, Lp + Lx + 1)
        ...
        return self.strip_eos(out[:, Lp + Lx:])  # k x (Lg_i,) response tokens y_0 .. y_{Lg-1}, right-pad dropped
    ...
    def sampling_step(self, epoch):
        ...
        for data_ids in tqdm(self.training_dataloader, desc="Sampling", position=1, leave=False, disable=not self.accelerator.is_main_process):
            ...
            prompt = self.build_prompt_tokens(self.task.prompt(data_id), system_prompt=self.task.SYSTEM_PROMPT, enable_thinking=cfg.enable_thinking)  # (1, Lp)

            response_tokens = self.generate(prompt.expand(k, -1))  # k x (Lg_i,), the same noise injected for all k
            ...
            training_data.append({
                "prompt_tokens": prompt.cpu(),                          # (1, Lp)
                "response_tokens": [t.cpu() for t in response_tokens],  # k x (Lg_i,)
    ...
    def training_step(self, epoch, training_data):
        N_local = sum(len(group["response_tokens"]) for group in training_data)  # this rank's rollouts
        ...
        for group in training_data:
            prompt_tokens = group["prompt_tokens"].to(self.model.device)  # (1, Lp)
            for tokens, advantage in zip(group["response_tokens"], group["advantages"]):
                ...
                logits = self.model(input_ids=tokens[None], past_key_values=cache, use_cache=True).logits  # (1, Lg, V)
                ...
                token_log_probs = log_probs.gather(-1, tokens[None, 1:, None]).squeeze(-1).squeeze(0)  # (Lg-1,)
        ...
        grad_norm = self.x.grad.norm(dim=-1).mean()  # L2 over D, averaged over the H*Lx vectors
        self.optimizer.step()
        with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            # subtracting it leaves the Lx*(Lx-1) ordered off-diagonal pairs.
            gram = self.x @ self.x.mT  # (H, Lx, Lx)
            Lx = gram.shape[-1]
            diag = torch.diagonal(gram, dim1=-2, dim2=-1)  # (H, Lx)
            pairwise_inner_product = ((gram.sum(dim=(-2, -1)) - diag.sum(-1)) / (Lx * (Lx - 1))).mean()
