...
def sample_noise(mu, sigma, batch_size, device):
    ...
    Args:
        mu, sigma: (H, Lx, D) mean / variance over the H layers x Lx noise tokens (each D-dim).
    Returns:
        noise: (N, H, Lx, D) raw samples mu + sqrt(sigma) * eps -- fed to the optimizer update.
        projected_noise: (N, H, Lx, D) sphere-projected samples -- fed to the KV injection.
    """
...
def update_parameters(mu, sigma, noise, objective_values, lr=1.0):
    ...
    loop, extended to our two leading axes: the H layers x Lx noise tokens are looped over (each
    (h, l) is its own independent D-dim Gaussian), and the hidden D is the single content axis, kept
    vectorized. Parameters stay in their original (H, Lx, D) shape -- no flattening.

    Args:
        mu, sigma: (H, Lx, D) parameters to update.
        noise: (H, Lx, N, D) samples that produced the objectives.
        objective_values: (H, Lx, N) score to minimize (i.e. -reward); one reward per sample, broadcast
            across the H x Lx slots.
    Returns:
        mu, sigma: (H, Lx, D) updated parameters.
    """
    ...
    H, Lx, N, D = noise.shape  # (H, Lx) are looped; D is the single hidden axis (vectorized)
...
class Trainer(BaseTrainer):
    """Noise-search baseline: optimize per-layer random KV rows -- Lx hidden-state vectors injected into
    ...
    def __init__(self, config):
        ...
        # 2. Shape (H, Lx, D): each of the H layers gets its own Lx noise tokens (each D-dim).
        ...
        self.prompt_tokens = self.build_prompt_tokens(user_prompt, system_prompt=system_prompt, enable_thinking=self.config.sample.enable_thinking)  # (1, Lp)
    ...
    def build_noisy_cache(self, x_noise):  # x_noise: (H, Lx, D) sphere-projected noise for one sample
        # Build the clean prompt KV cache, then inject Lx "random" KV rows into every layer -- each row
        ...
        # model's true K/V manifold (cf. test-qwen-noise.py Stage 2). Returns (cache, prompt_logits, Lp).
        Lx = self.config.sample.noise_length
        out = self.model(input_ids=self.prompt_tokens, use_cache=True, logits_to_keep=1)  # only the last row is read
        cache = out.past_key_values
        Lp = cache.get_seq_length()
        pos = torch.arange(Lp, Lp + Lx, device=self.model.device).unsqueeze(0)  # positions Lp .. Lp+Lx-1
        rotary = self.model.model.rotary_emb
        for h, layer in enumerate(self.model.model.layers):
            attn = layer.self_attn
            dev = attn.k_proj.weight.device
            Dh = attn.head_dim
            x = x_noise[h][None].to(dev, attn.k_proj.weight.dtype)  # (1, Lx, D)
            ...
            k = einops.rearrange(k, "b l h d -> b h l d")               # (1, heads_kv, Lx, Dh)
            v = einops.rearrange(attn.v_proj(xln), "b l (h d) -> b h l d", d=Dh)  # (1, heads_kv, Lx, Dh)
            cos, sin = rotary(v, pos.to(dev))
            k, _ = apply_rotary_pos_emb(k, k, cos.to(dev), sin.to(dev))  # RoPE at positions Lp .. Lp+Lx-1
            cl = cache.layers[h]
            cl.keys = torch.cat([cl.keys, k.to(cl.keys.device)], dim=2)
            cl.values = torch.cat([cl.values, v.to(cl.values.device)], dim=2)
        return cache, out.logits, Lp
    ...
    def generate(self, x_noise):
        ...
        # logits (it does not attend to the noise: causality forbids position Lp-1 from seeing rows at
        # >=Lp); every later token is generated at position >= Lp+Lx and does attend over the random KV.
        # Padding the input to Lp+Lx+1 makes `generate` feed only first_token (at position Lp+Lx) and take
        # over -- the Lx placeholders stand in for the injected KV rows and are never embedded.
        Lx = self.config.sample.noise_length
        cache, prompt_logits, Lp = self.build_noisy_cache(x_noise)
        first_token = prompt_logits[:, -1:, :].argmax(-1)  # (1, 1)
        placeholders = self.prompt_tokens.new_full((1, Lx), self.tokenizer.pad_token_id)
        input_ids = torch.cat([self.prompt_tokens, placeholders, first_token], dim=1)  # (1, Lp + Lx + 1)
        ...
        return self.tokenizer.decode(out[0, Lp + Lx:], skip_special_tokens=True)

    @torch.no_grad()
    def sampling_step(self, epoch):
        noise, projected_noise = sample_noise(self.mu, self.sigma, self.N_local, self.accelerator.device)  # (N_local, H, Lx, D)

        responses = []
        rewards = []
        for i in range(self.N_local):
            response = self.generate(projected_noise[i])  # (H, Lx, D)
        ...
        gathered_noise     = self.accelerator.gather(noise)    # (N, H, Lx, D) raw noise, gathered along the sample axis
        ...
        # Per-slot Gaussian update over the (H, Lx) leading axes (mu kept in its original (H, Lx, D) shape).
        H, Lx, _ = self.mu.shape
        objective_values = (-gathered_rewards)[None, None].expand(H, Lx, -1)    # (H, Lx, N), shared across all slots
        noise_hlnd = einops.rearrange(gathered_noise, "N H Lx D -> H Lx N D")
